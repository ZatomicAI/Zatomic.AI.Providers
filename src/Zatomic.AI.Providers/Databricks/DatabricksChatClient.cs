...
{
	public class DatabricksChatClient : BaseClient
	{
		public string ApiKey { get; set; }
		public string ApiUrl { get; set; }

		public DatabricksChatClient()
		{
		}

		public DatabricksChatClient(string apiKey) : this()
		{
			ApiKey = apiKey;
		}

		public async Task<DatabricksChatResponse> ChatAsync(DatabricksChatRequest request)
		{
			DatabricksChatResponse response = null;
			...
			using (var httpClient = new HttpClient())
			{
				...
				try
				{
					...
					response = responseJson.Deserialize<DatabricksChatResponse>();
					response.Duration = stopwatch.ToDurationInSeconds(2);
				}
				catch (Exception ex)
				{
					var aiEx = AIExceptionUtility.BuildDatabricksAIException(ex, request, responseJson);
					throw aiEx;
				}
			}
		...
		public async IAsyncEnumerable<AIStreamResponse> ChatStreamAsync(DatabricksChatRequest request)
		{
			request.Stream = true;
			...
			using (var httpClient = new HttpClient())
			{
				...
				{
					var aiEx = AIExceptionUtility.BuildDatabricksAIException(ex, request);
					throw aiEx;
				}
				...
				using (var reader = new StreamReader(stream))
				{
					while (!streamComplete)
					{
						...
						{
							var rsp = line.Substring(6).Deserialize<DatabricksChatResponse>();
