...
{
	public class FireworksAIClient
	{
		public string ApiKey { get; set; }
		public string ApiUrl { get; set; } = "https://api.fireworks.ai/inference/v1/chat/completions";

		public FireworksAIClient()
		{
		}

		public FireworksAIClient(string apiKey) : this()
		{
			ApiKey = apiKey;
		}

		public async Task<FireworksAIResponse> ChatAsync(FireworksAIRequest request)
		{
			FireworksAIResponse response = null;
			...
			using (var httpClient = new HttpClient())
			{
				...
				try
				{
					...
					response = responseJson.Deserialize<FireworksAIResponse>();
					response.Duration = stopwatch.ToDurationInSeconds(2);
				}
				catch (Exception ex)
				{
					var aiEx = AIExceptionUtility.BuildFireworksAIAIException(ex, request, responseJson);
					throw aiEx;
				}
			}
		...
		public async IAsyncEnumerable<AIStreamResult> ChatStreamAsync(FireworksAIRequest request)
		{
			request.Stream = true;
			...
			using (var httpClient = new HttpClient())
			{
				...
				{
					var aiEx = AIExceptionUtility.BuildFireworksAIAIException(ex, request);
					throw aiEx;
				}
				...
				using (var reader = new StreamReader(stream))
				{
					while (!reader.EndOfStream && !streamComplete)
					{
						...
						// Fireworks AI streams event lines with "data: ", so that's why we substring the line at 6
						if (!line.IsNullOrEmpty() && line.StartsWith("data: "))
						{
							var rsp = line.Substring(6).Deserialize<FireworksAIResponse>();
