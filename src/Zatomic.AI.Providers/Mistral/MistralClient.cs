...
namespace Zatomic.AI.Providers.Mistral
{
	public class MistralClient
	{
		...
		public MistralClient(string apiKey) : this()
		{
			ApiKey = apiKey;
		}

		public async Task<MistralResponse> ChatAsync(MistralRequest request)
		{
			MistralResponse response = null;
			...
			using (var httpClient = new HttpClient())
			{
				...
				try
				{
					...
					response = responseJson.Deserialize<MistralResponse>();
		...
		public async IAsyncEnumerable<AIStreamResult> ChatStreamAsync(MistralRequest request)
		{
			request.Stream = true;
			...
			using (var httpClient = new HttpClient())
			{
				...
				using (var reader = new StreamReader(stream))
				{
					while (!reader.EndOfStream && !streamComplete)
					{
						...
						{
							var rsp = line.Substring(6).Deserialize<MistralResponse>();
