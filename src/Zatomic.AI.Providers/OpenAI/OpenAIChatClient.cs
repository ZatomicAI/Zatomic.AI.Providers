...
{
	public class OpenAIChatClient : BaseClient
	{
		public string ApiKey { get; set; }
		public string ApiUrl { get; } = "https://api.openai.com/v1/chat/completions";

		public OpenAIChatClient()
		{
		}

		public OpenAIChatClient(string apiKey) : this()
		{
			ApiKey = apiKey;
		}

		public async Task<OpenAIChatResponse> ChatAsync(OpenAIChatRequest request)
		{
			...
			using (var httpClient = new HttpClient())
			{
				...
				try
				{
					...
					response = responseJson.Deserialize<OpenAIChatResponse>();
					response.Duration = stopwatch.ToDurationInSeconds(2);
				}
				catch (Exception ex)
				{
					var aiEx = AIExceptionUtility.BuildOpenAIAIException(ex, request, responseJson);
					throw aiEx;
				}
			}
		...
		public async IAsyncEnumerable<AIStreamResponse> ChatStreamAsync(OpenAIChatRequest request)
		{
			request.Stream = true;
			request.StreamOptions = new OpenAIChatStreamOptions { IncludeUsage = true };
			...
			using (var httpClient = new HttpClient())
			{
				...
				{
					var aiEx = AIExceptionUtility.BuildOpenAIAIException(ex, request);
					throw aiEx;
				}
				...
				using (var reader = new StreamReader(stream))
				{
					while (!streamComplete)
					{
						...
						if (!line.IsNullOrEmpty() && line.StartsWith("data: "))
						{
							...
							var rsp = line.Substring(6).Deserialize<OpenAIChatResponse>();
